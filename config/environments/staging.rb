Rails.application.configure do
  ...
  config.action_controller.asset_host = 'staging.clientes.shipit.cl'
  ...
  # config.active_job.queue_name_prefix = "shipit-staff_#{Rails.env}"
  ...
  config.action_mailer.default_url_options = { host: 'staging.clientes.shipit.cl' }
  ...
  config.action_cable.url = 'ws://staging.clientes.shipit.cl/cable'

  config.opit_endpoint = 'http://staging.opit.shipit.cl'
  config.fulfillment_endpoint = 'http://staging.fulfillment.shipit.cl/api/'
  config.statuses_endpoint = 'http://courierstatus.shipit.cl/v/'

  config.action_cable.allowed_request_origins = ["http://staging.clientes.shipit.cl", "https://staging.clientes.shipit.cl"]
  ...
  config.rabbitmq = {
    protocol: 'amqps',
    host: ENV.fetch('SHIPIT_RABBIT_HOST') { 'b-805c78b3-74e2-4b51-8251-b7bee97d9d1e.mq.us-west-2.amazonaws.com' },
    port: ENV.fetch('SHIPIT_RABBIT_PORT') { 5671 },
    heartbeat: 1,
    daemonize: true,
    workers: 1,
    log: "#{Rails.root}/log/workers.log",
    pid_path: '/home/deploy/www/clientes/shared/tmp/pids/workers.pid',
    prefetch: 1,
    threads: 1,
    user: ENV.fetch('SHIPIT_RABBIT_USERNAME') { 'shipit_staging' },
    password: ENV.fetch('SHIPIT_RABBIT_PASSWORD') { 'yF3ez2mLUpdbQT9S' }
  }
  ...
  config.database = {
    host: ENV['SHIPIT_CORE_DATABASE_HOST'],
    database_name: ENV['SHIPIT_CORE_DATABASE_NAME'],
    username: ENV['SHIPIT_CORE_DATABASE_USERNAME'],
    password: ENV['SHIPIT_CORE_DATABASE_PASSWORD'],
  ...
  config.prices_url = 'http://staging.prices.shipit.cl/v'
  config.order_endpoint = 'http://staging.orders.shipit.cl/v'
  config.elastic_api_url = 'http://staging.elastic.shipit.cl/v'
  ...
  config.redis_url = 'redis://suite.nmc4qr.ng.0001.usw2.cache.amazonaws.com:6379'
  config.redis_host = ''
  config.wicked_pdf_conf = { exe_path: '/usr/local/bin/wkhtmltopdf' }
  ...
  config.internal = {
    url: ENV.fetch('INTERNAL_URL') {'http://staging.internal.shipit.cl/v'},
