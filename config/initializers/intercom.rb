IntercomRails.config do |config|
  ...
  config.user.custom_data = {
    first_name: ->(current_account) { current_account.person.first_name },
    last_name: ->(current_account) { current_account.person.last_name },
    phone: ->(current_account) { current_account.entity.phone },
    packages: ->(current_account) { current_account.current_company.packages.count }
  }
  ...
  #
  config.company.current = Proc.new { current_account.entity }
  #
  ...
  config.company.custom_data = {
    branch_offices: ->(app) { app.actable_type.include?('Company') ? app.specific.branch_offices.count : 0 },
    website: ->(app) { app.specific.website },
    contact_email: ->(app) { app.specific.email_contact },
    run: ->(app) { app.specific.run },
    phone: ->(app) { app.specific.phone },
    commune: ->(app) { app.address.commune.try(:name) },
    region: ->(app) { app.address.commune.try(:region).try(:name) },
    fulfillment: ->(app) { app.actable_type.include?('Company') ? app.specific.fulfillment? : false },
    any_integrations: ->(app) { app.actable_type.include?('Company') ? app.specific.any_integrations? : false },
    last_delivery_date: ->(app) { app.actable_type.include?('Company') ? app.specific.last_delivery_date : '' },
    category: ->(app) { app.actable_type.include?('Company') ? app.specific.category : '' },
    priority: ->(app) { app.actable_type.include?('Company') ? app.specific.priority : '' },
    need_key_account_manager: ->(app) { app.actable_type.include?('Company') ? app.specific.need_key_account_manager : '' },
    key_account_manager: ->(app) { app.actable_type.include?('Company') ? app.specific.key_account_manager : '' },
    packages_hope: ->(app) { app.actable_type.include?('Company') ? app.specific.packages_hope : '' },
    last_month_deliveries: ->(app) { app.actable_type.include?('Company') ? app.specific.last_month_deliveries : '' },
    sellers: ->(app) { app.actable_type.include?('Company') ? Setting.fullit(app.specific.id).try(:sellers_availables).try(:join, ', ') : '' },
    active: ->(app) { app.actable_type.include?('Company') ? app.specific.active : '' },
    debtors: ->(app) { app.actable_type.include?('Company') ? app.specific.debtors : '' },
  }
