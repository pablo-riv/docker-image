class V::V4::SubscriptionsController < V::ApplicationController
  ...
  def create
    @subscription = Subscriptions::SubscriptionService.new(@company.id, subscriptions_params).generate
  ...
  def update
    Subscription.transaction do
      raise 'Error actualizando suscripción' unless @subscription.update_columns(prices: subscriptions_params[:prices],
  ...
  def change_plan
    Subscription.transaction do
      ...
      raise 'Error actualizando suscripción' unless @subscription.update_columns(is_active: false)
  ...
  def activate
    Subscription.transaction do
      raise 'Error activando suscripción' unless @subscription.update_attribute(:is_active, true)
